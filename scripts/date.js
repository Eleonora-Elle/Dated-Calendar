...
export function generateMonthCalendarDays(currentDate) {
  ...
  const lastDayOfPreviousMonthWeekDay = lastDayOfPreviousMonthDate.getDay() || 7;
  if (lastDayOfPreviousMonthWeekDay !== 7) {
    for (let i = lastDayOfPreviousMonthWeekDay - 1; i >= 0; i -= 1) {
...
export function generateWeekDays(date) {
  ...
  const firstWeekDay = subtractDays(date, day - 1);
